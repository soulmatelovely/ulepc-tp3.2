...
define([
    'backbone',
    'models/youleGame/change',
    'text!templets/youleGame/change.html',
    'uploadify',
    'select2',
    'utils/ueditorUtil'
], function(
    ...
    var view = Backbone.View.extend({
        el: '#CONTENT',
        events: {
            'click [role=saveChange]': 'saveFun', //保存
            ...
            var $myModal = $('#myModal'),
                ...
                Model.fetchInfo({
                        ...
                        success: function(map) {
                            ...
                            $('input[name=apkname]').val(map.apkname);
                            $('input[name=apk_size]').val(map.apk_size);
                            $('input[name=people]').val(map.people);
                            $('input[name=versioninfo]').val(map.versioninfo);
                            $('input[name=developer]').val(map.developer);
                            _this.UMinit();

                            $("select[name=comment]").val(map.comment);

                            if(map.ishotgame=='1'){
                                $('input[name=apptype1]').attr('checked',true);
                            }
                            if(map.isinternetgame=='1'){
                                $('input[name=apptype2]').attr('checked',true);
                            }
                            if(map.isphonegame=='1'){
                                $('input[name=apptype3]').attr('checked',true);
                            }
                            if(map.ispcgame=='1'){
                                $('input[name=apptype4]').attr('checked',true);
                            }

                              // 新增
                            if(map.is_download_notice=='1'){
                                $('input[name=ranking1]').attr('checked',true);
                            }
                            if(map.is_soaring_notice=='1'){
                                $('input[name=ranking2]').attr('checked',true);
                            }
                            
                            $("input[name=istuijian][value='"+map.isgame+"']").attr('checked','checked');

                            map.picture == '' || _this.imgEcho($('input[name=picture]'), map.picture);

                            $('textarea[name=introduce]').val(map.introduce);

                            //多图渲染
                            var screenshot = map.screenshot;
                            if(screenshot == null){
                                screenshot = '';
                            }else if(screenshot != null){
                                screenshot = map.screenshot.split(',');
                            }
                            var length = screenshot.length;
                            var imgList='';
                            var multi=$('.multi-warp');
                            for(var i=0;i<length;i++){
                                imgList += '<div  style="width:140px; height:90px" class="img-list">';
                                imgList += '<input role="img-list"  name="screenshot[]"  type="hidden" value="'+ screenshot[i] +'">';
                                imgList += '<img  style="width:140px; height:90px" src="' + 'http://shihai.oss-cn-beijing.aliyuncs.com/'  + screenshot[i] + '">';
                            ...
                            multi.html(imgList);
                             ...
                             //渲染apk
                             map.apk == '' || _this.imgEcho($('input[name=apk]'), '1F414152K94R.jpg');
                             $('input[name=apk]').val(map.apk);
